...
macro_rules! gen_types {
    ($Types:ident $ConstructorTypes:ident $BufferTypes:ident $VecTypes:ident, $($type:ident $type_ref:ident $ctype:ident $ctype_ref:ident $btype:ident $btype_ref:ident $vtype:ident $vtype_ref:ident|$type_rust:ty|$len:literal|$type_ocl:ident|$type_opencl:literal) +) => {
        ...
        impl $Types {
            ...
            pub fn set_arg(&self, kernel: &(ocl::Kernel,std::collections::BTreeMap<String,u32>), kernel_name: &str, param: &str) -> crate::gpgpu::Result<()> {
                match self {
            ...
            pub fn gen_buffer(&self, pq: &ocl::ProQue, len: usize) -> crate::gpgpu::Result<$BufferTypes> {
                match self {
                    $(Self::$type(v) =>
        ...
        impl $BufferTypes {
            ...
            pub fn copy(&self, to: &$BufferTypes) -> crate::gpgpu::Result<()>{
                match self {
                    $(Self::$btype(v) => {
            ...
            pub fn get(&self) -> crate::gpgpu::Result<$VecTypes> {
                match self {
                    $(Self::$btype(v) => {
                        let buf = v;
                        let len = buf.len();
            ...
            pub fn get_first(&self) -> crate::gpgpu::Result<$Types> {
                match self {
                    $(Self::$btype(v) => {
                        let buf = v;
            ...
            pub fn get_firsts(&self, num: usize) -> crate::gpgpu::Result<$VecTypes> {
                match self {
                    $(Self::$btype(v) => {
                        let buf = v;
        ...
        impl $VecTypes {
            ...
            pub fn gen_buffer(&self, pq: &ocl::ProQue) -> crate::gpgpu::Result<$BufferTypes> {
                match self {
                    $(Self::$vtype(v) =>
                        Ok($BufferTypes::$btype(pq.buffer_builder()
                       .len(v.len())
