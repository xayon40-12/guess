...
use crate::gpgpu::descriptors::{KernelArg,BufferTypes,VecTypes,Types};
use crate::gpgpu::algorithms::{Callback,AlgorithmParam};
...
impl Handler {
    ...
    fn set_kernel_arg_buf(&self, name: &str, kernel: &(Kernel,BTreeMap<String,u32>), n: &str, m: &str) -> crate::gpgpu::Result<()>{
    ...
    fn _set_arg(&self, name: &str, desc: &[KernelArg], kernel: &(Kernel,BTreeMap<String,u32>)) -> crate::gpgpu::Result<()> {
        for d in desc {
            match d {
                KernelArg::Param(n,v) =>
    ...
    pub fn run_algorithm(&mut self, name: &str, dim: Dim, dimdir: &[DimDir], bufs: &[&str], other_args: AlgorithmParam) -> crate::gpgpu::Result<Option<Box<dyn Any>>> {
        (self.algorithms.get(name).expect(&format!("Algorithm \"{}\" not found",name)).clone())(self,dim,dimdir,bufs,other_args)
    }

    pub fn copy(&mut self, src: &str, dst: &str) -> crate::gpgpu::Result<()> {//TODO add verbosity to error for copy (names of the buffers)
